...
class App(QDialog):
    def __init__(self):
       ...
       # self.updateArduinoUI()
        ...
        self.pybpod_variables_to_display = ['ValveOpenTime_L',
                                            ...
                                            'ITI_min',
                                            'ITI_max',
                                            'RewardOmissionRatio', # proportion of trials without reward
                                            'CueOmissionRatio', # proportion of trials without go cue
                                            'NoLickTime', #at the start of the trial before GO cue
                                            'TimeToReward',
                                            'GoCueLength',#seconds
                                            'RewardConsumeTime',
                                            'RecordMovies',
    ...
    def bpod_load_parameters(self):
        ...
        if project_now != '?' and experiment_now != '?' and setup_now != '?' and subject_now != '?':
            subject_var_file = os.path.join(self.pybpod_dir,project_now,'subjects',subject_now,'variables_pavlovian.json')
    ...
    def bpod_loaddirectorystructure(self,projectnames_needed = None, experimentnames_needed = None,  setupnames_needed=None):
        dirstructure, projectnames, experimentnames, setupnames, sessionnames, subjectnames_ = utils_pybpod.loaddirstucture(self.pybpod_dir,projectnames_needed, experimentnames_needed,  setupnames_needed)
        subjectnames = []
        for subjectname in subjectnames_:
            if 'PKJ' in subjectname:
...
class PlotCanvas(FigureCanvas):
    ...
    def update_bpod_plot(self,data):
        ...
        licks_t = []
        licks_trial = []
        for trial, (licks,rewards,gocues) in enumerate(zip(data['lick_L'],data['reward_L'],data['go_cue_times'])):
        ...
        self.ax1.plot(np.concatenate(licks_t),np.concatenate(licks_trial),'k.',markersize = 1)
        self.ax1.plot(go_t,go_trial,'g.')
        self.ax1.plot(go_omission_t,go_omission_trial,'kx')
        self.ax1.plot(reward_t,reward_trial,'r.')
        self.ax1.plot(reward_omission_t,reward_omission_trial,'kx')
        ...
        self.ax1.set_ylabel('Trial #')
        self.ax1.set_xlabel('Time from GO cue (s)')
        self.ax1.hlines(file_start_trialnum, -2,10, colors='b', linestyles='dashed')
        self.ax1.set_title('Trials: {} Hits: {}'.format(len(trial_hit[file_start_trialnum[-1]:]),sum(trial_hit[file_start_trialnum[-1]:])))
        self.ax1.set_xlim([-5,10])
        self.draw()
